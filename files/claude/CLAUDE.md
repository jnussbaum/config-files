...
- When writing comments, avoid referring to temporal context about recent changes.
...
- Don't ignore the output of the system or the tests - Logs and messages often contain critical information.
...
When encountering tool failures (ruff, pytest, etc.), remember: 
...
- Never push unless I explicitly ask.
  - Exception: If pushing is part of the workflow of the `eng` plugin of the dasch-claude-plugins marketplace, then do it without my approval.
- When I tell you to open a PR, always assign it to me (https://github.com/jnussbaum).
- When writing PR descriptions, never focus on commit history, but only focus on actual code/diff.

Conditional rules and stack-specific exceptions live in [tech-preferences/git.md](tech-preferences/git.md).

## Technology Preferences

Tech-specific instructions live in separate documents.
...
- **[Claude Code components](tech-preferences/claude-code-components.md)** —
  when writing skills, agents, commands, and hooks
...
- Use your built-in memory system only for facts specific to a single repo
  that are not derivable from its code.
